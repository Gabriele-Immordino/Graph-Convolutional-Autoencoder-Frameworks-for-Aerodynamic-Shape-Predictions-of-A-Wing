...
class MLS_interpolation:
    ...
    def __init__(self, x_src, y_src, z_src, x_dst, y_dst, z_dst):
        """
        Initialize the MLS_interpolation object with source and destination coordinates.
    ...
    def gaussian_weight(self, distance, h):
        ...
        Args:
            distance (float or np.ndarray): Distance(s) to compute the weight for.
            h (float): Smoothing parameter.

        Returns:
            np.ndarray: Gaussian weights.
        """
        return np.exp(-distance**2 / h**2)  # Compute Gaussian weight

    def polynomial_basis(self, x):
        """
        Compute the polynomial basis vector for 3D coordinates.

        Args:
            x (array-like): 3D coordinates (x, y, z).

        Returns:
            np.ndarray: Polynomial basis vector.
        """
        # Return the polynomial basis vector for the input coordinates
        return np.array([
            x[0]**0,      # 1
            x[0],         # x
            x[1],         # y
    ...
    def calculate_distances(self):
        ...
        """
        src_points = np.array(list(zip(self.x_src, self.y_src, self.z_src)))  # Stack source points
        dst_point = np.array([self.x_dst, self.y_dst, self.z_dst])            # Destination point
    ...
    def interpolation(self, dist=None):
        ...
        Args:
            dist (list, optional): Precomputed distances. If None, distances are calculated.

        Returns:
            list: List of torch tensors representing the interpolation coefficients.
        """
        if dist:
        ...
        a = self.polynomial_basis((self.x_dst, self.y_dst, self.z_dst))  # Basis for destination point
        b = np.linalg.pinv(np.dot(P.T, np.dot(W, P)))  # Pseudo-inverse for weighted least squares
        c = np.dot(P.T, W)  # Weighted design matrix
